...
- **Two pixel-art views** (drawn from photos of the real hardware, generated with Codex). The front view shows the console's port (ivory top / maroon bottom shell, 8 holes on the top row = odd pins, 7 on the bottom = even pins; hover a hole for its description). The top-down view shows the MIWA key sticking out of the console's front edge.
- **Turn it** — the same twist as unlocking a door. Use the slider (±30°): in the front view the blade's cross-section turns like a clock hand, and **turning right dips the blade's right end onto the bottom row while turning left lifts the left end onto the top row**. In the top-down view the key visibly narrows as it turns. **Drag sideways** for the insertion position.
- **Rattle it** injects about a second of chatter (1,789,773 CPU cycles). Shorted pins light up and the key shakes in both views. Slow the clock down and the burst stretches in real time.
...
Two homebrew test ROMs are bundled so you can see the effect (generated by `tools/make_*_rom.py`, freely redistributable):

- [`web/exptest.nes`](web/exptest.nes) (`?rom=/exptest.nes`) — visualizes the raw expansion-port reads as the background color: black = quiet, white = $4016 D1, red = $4017 D0-D4, green = both.
- [`web/exppitch.nes`](web/exppitch.nes) (`?rom=/exppitch.nes`) — a mini pitching demo using Nintendo's standard D0|D1 OR read routine. Untouched, the ball flies straight; rattle the key and it visibly curves.
