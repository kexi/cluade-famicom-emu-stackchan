...
The whole toolchain — clang, Emscripten, PlatformIO, Python — comes from a nix flake. With [nix](https://nixos.org) and [direnv](https://direnv.net) installed, `cd` into the repo and everything is provided; otherwise prefix commands with `nix develop --command`.
