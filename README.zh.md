...
- **两个视角的像素画**(参照实机照片、由 Codex 生成)。"正面视图"是主机前脸的端口(上半部象牙白/下半部枣红,上排 8 孔=奇数引脚、下排 7 孔=偶数引脚,悬停可查看说明);"俯视图"是插在主机前缘的 MIWA 钥匙。
- **旋转** — 和开门锁一样的拧法。用滑块(±30°)旋转:正面视图里刀身截面像时钟指针一样转动,**向右拧时刀身右端压到下排、向左拧时左端抬到上排**的引脚。俯视图里钥匙会随之变窄。**左右拖动**调整插入位置。
- **乱捅** — 注入约 1 秒(1789773 个 CPU 周期)的抖动。短路中的引脚会发光,两个视图中的钥匙都会晃动。降低时钟频率时,这段噪声在真实时间里会相应变长。
- 碰到 3 号引脚(/IRQ)会随机触发中断,导致画面花屏或跑飞(按复位可恢复)。/OE、OUT、SOUND 和 +5V 未建模,只提供说明文字。

随附两个自制测试 ROM,可直接体验效果(由 `tools/make_*_rom.py` 生成,可自由再分发):

- [`web/exptest.nes`](web/exptest.nes)(`?rom=/exptest.nes`) — 用背景颜色可视化扩展端口的原始读值:黑=无噪声,白=$4016 D1,红=$4017 D0-D4,绿=两者。
- [`web/exppitch.nes`](web/exppitch.nes)(`?rom=/exppitch.nes`) — 迷你投球演示,实现了任天堂标准的 D0|D1 OR 读取例程。不动它球是直线;摇晃钥匙时球会明显变化(变化球)。
