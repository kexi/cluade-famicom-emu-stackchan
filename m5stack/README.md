...
同一オリジンへの POST を `tools/serve_web.py` が UDP パケットへ中継する。

| エンドポイント | 中継先 |
| --- | --- |
| `POST /api/pins` `{host, mask}` | type 1 (端子状態) |
| `POST /api/reset` `{host}` | type 2 (RESET) |
| `POST /api/volume` `{host, volume}` | type 2 (音量) |
| `POST /api/pad` `{host, pad1, pad2}` | type 0 (パッド) |

## ブラウザをコントローラーにする

ミラーモード中は、ブラウザのキーボード・Gamepad API・画面上のタッチパッドの
入力がそのまま実機のコントローラーになる。押した/離した瞬間は即座に送信し、
状態が変わらない間は 250ms ごとにハートビートを送る (ファーム側の
`INPUT_TIMEOUT_MS` = 500ms の半分なので、1発落ちても誤解放しない)。

エミュレーションを止めると送信も止まるため、500ms 後に実機側は全ボタン解放に
戻る。これは意図した安全動作。

TAS 再生中の入力もミラーされる。実機で TAS が再生される様子を眺められる。

スマホをコントローラーにするには LAN に公開する:

```sh
uv run tools/serve_web.py --bind 0.0.0.0
# スマホで http://<Mac の IP>:8000/?device=<CoreS3 の IP>
```

> **注意**: `--bind 0.0.0.0` は同一ネットワークの誰でも実機を操作でき、
> 中継サーバー経由で任意のホストにパケットを送れる状態になる。
> 信頼できるネットワークでのみ使うこと。デフォルトは 127.0.0.1。

プロコン (`tools/procon_udp.py`) と同時に使った場合、どちらも type 0 パケットを
送るため **last-write-wins** になる。ブラウザ側はハートビートを送り続けるので、
両方を同時に動かすと入力が奪い合いになる。片方だけ使うこと。
